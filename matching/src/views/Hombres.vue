<template>
  <div class="about">
    ...
    <b-input-group class="d-flex justify-content-center">
      <b-form-input v-model="nombre" placeholder="Hombre" class="col-sm-3"></b-form-input>
...
export default {
  ...
  methods: {
    buscar: function(e) {
      const request = `http://localhost/prolog/hombre?nombre=${this.nombre}`;
      axios
        .get(request)
        .then(res => {
          const bruto = res.data;
          //Limpiar arreglo
          this.datos = this.getUnique(bruto, "mujer");
        })
        .catch(err => {
